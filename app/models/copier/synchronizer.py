...
def get_notification_settings() -> dict:
    """Получить настройки уведомлений из конфига"""
...
class PositionSynchronizer:
    ...
    def sync(
        ...
        logger.info("=" * 60)
        logger.info(f"{mode_icon} Starting synchronization")
        logger.info(f"   Mode: {mode_str}")
        if self.operating_mode == 'simulation':
            logger.info(f"   ⚠️ SIMULATION: Orders will NOT be sent to Schwab")
        logger.info("=" * 60)
        ...
        try:
            ...
            if not deltas:
                logger.info("✅ All positions are in sync, no orders needed")
            ...
            logger.info("=" * 60)
            logger.info(f"✅ Synchronization complete: {len(results)} orders")
            logger.info("=" * 60)
    ...
    def _execute_orders(
        ...
        logger.info(f"🔴 Executing {len(deltas)} LIVE orders...")
        ...
        for symbol, delta in deltas.items():
            ...
            if api_error is None:
                ...
                logger.info(f"   🔴 ✅ {action} {quantity} {symbol} @ "
                ...
                logger.error(f"   🔴 ❌ {action} {quantity} {symbol}: {api_error.message}")
        ...
        if error_summary['total_errors'] > 0:
            logger.warning(f"⚠️ Order execution completed with {error_summary['total_errors']} errors")
    ...
    def _dry_run_orders(
        ...
        logger.info(f"🧪 DRY RUN: {len(deltas)} orders (not executed)")
        ...
        for symbol, delta in deltas.items():
            ...
            logger.info(f"   🧪 {action} {quantity} {symbol} @ ${prices.get(symbol, 0):.2f}")
    ...
    def _simulate_live_orders(
        ...
        """
        logger.info(f"🔶 LIVE SIMULATION: {len(deltas)} orders")
        logger.info("=" * 50)
        logger.info("⚠️  SIMULATION MODE - NO ORDERS SENT TO SCHWAB")
        logger.info("=" * 50)
        ...
        for symbol, delta in deltas.items():
            ...
            logger.info(f"   🔶 Would {action} {quantity} {symbol} @ ${price:.2f} = ${value:,.2f}")
            logger.info(f"      Order ID: {sim_order_id}")
            logger.info(f"      ✅ Validated, ready to execute")

        logger.info("=" * 50)
        logger.info(f"📊 SIMULATION SUMMARY:")
        logger.info(f"   Total BUY:  ${total_buy:,.2f}")
        logger.info(f"   Total SELL: ${total_sell:,.2f}")
        logger.info(f"   Net:        ${total_buy - total_sell:,.2f}")
        logger.info("=" * 50)
        logger.info("⚠️  To execute real orders, disable simulation mode")
    ...
    def _save_sync_result(self, result: Dict):
        ...
        try:
            ...
            logger.info(f"💾 Results saved to {history_file} ({mode_str})")
