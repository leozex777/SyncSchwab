...
def render():
    ...
    with col1:
        ...
        if st.button("📊 Account Information", type=btn_type1, use_container_width=True, key="detail_tab_account"):
            st.session_state.client_details_tab = 'Account Information'
            st.rerun()

    with col2:
        btn_type2 = "primary" if current_tab == 'Client Settings' else "secondary"
        if st.button("⚙️ Client Settings", type=btn_type2, use_container_width=True, key="detail_tab_settings"):
            st.session_state.client_details_tab = 'Client Settings'
            st.rerun()

    with col3:
        btn_type3 = "primary" if current_tab == 'History' else "secondary"
        if st.button("📈 History", type=btn_type3, use_container_width=True, key="detail_tab_history"):
            st.session_state.client_details_tab = 'History'
            st.rerun()

    with col4:
        btn_type4 = "primary" if current_tab == 'Optional Tab' else "secondary"
        if st.button("ℹ️ Optional Tab", type=btn_type4, use_container_width=True, key="detail_tab_optional"):
...
def _render_client_history(selected):
    """История синхронизаций клиента"""

    st.markdown("**Synchronization History**")

    from app.core.json_utils import load_json

    history_file = selected.settings.get('history_file', f'data/clients/{selected.id}_history.json')
    ...
    try:
        ...
        if history:
            ...
            for h in reversed(history[-20:]):
                ...
                total_deltas = summary.get('total_deltas', len(h.get('deltas', [])))
                orders_placed = summary.get('orders_placed', len(h.get('results', [])))
