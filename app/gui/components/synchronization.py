...
def render():
    ...
    with col1:
        ...
        if st.button("⚙️ Auto Sync", type=btn_type1, use_container_width=True, key="sync_tab_auto"):
            st.session_state.sync_panel_tab = 'Auto Sync'
            st.rerun()

    with col2:
        btn_type2 = "primary" if current_tab == 'Manual Sync' else "secondary"
        if st.button("✏️ Manual Sync", type=btn_type2, use_container_width=True, key="sync_tab_manual"):
            st.session_state.sync_panel_tab = 'Manual Sync'
            st.rerun()

    with col3:
        btn_type3 = "primary" if current_tab == 'Optional Tab' else "secondary"
        if st.button("ℹ️ Optional Tab", type=btn_type3, use_container_width=True, key="sync_tab_optional"):
...
def _calculate_next_sync(last_sync_time: str, interval_str: str) -> str:
    ...
    try:
        ...
        # Сколько секунд осталось до следующей синхронизации
        remaining = interval_seconds - (elapsed % interval_seconds)
        
        # Если осталось меньше 2 секунд - показать "Now"
        if remaining < 2:
            return "Now"
...
def _render_auto_sync():
    ...
    with col_middle:
        ...
        # Заголовок Status
        st.caption("Status (updates every 10 sec)")
    ...
    with col_save:
        if st.button("💾 Save Synchronization", type="primary", use_container_width=True, key="auto_save_btn"):
            new_settings = settings.copy()
            new_settings['auto_sync_all_enabled'] = enable_all
            new_settings['auto_selected_clients'] = selected_clients if not enable_all else []
            new_settings['auto_sync_interval'] = interval
            new_settings['auto_sync_market_hours'] = market_hours

            if not market_hours:
                new_settings['auto_sync_start_time'] = start_time.strftime("%H:%M")
                new_settings['auto_sync_end_time'] = end_time.strftime("%H:%M")

            _save_sync_settings(new_settings)
            st.toast("✅ Settings saved!")
            st.rerun()

    with col_reset:
        if st.button("🔄 Reset Auto Sync", use_container_width=True, key="auto_reset_btn"):
...
def _render_status_fragment():
    ...
    Обновляется каждые 10 секунд независимо от остальной страницы.
    """
    import json

    # Читать напрямую без логирования
    try:
    ...
    with col_value:
        if is_running:
            # AUTO режим - показывать реальные значения
            st.markdown(f"**{_format_last_sync_time(last_sync_time)}**")
            st.markdown(f"**{_calculate_next_sync(last_sync_time, current_interval)}**")
...
def _render_manual_sync():
    ...
    with col_save:
        if st.button("💾 Save Synchronization", type="primary", use_container_width=True, key="manual_save_btn"):
            new_settings = settings.copy()
            new_settings['sync_all_enabled'] = sync_all
            new_settings['selected_clients'] = selected_clients if not sync_all else []
    ...
    with col_reset:
        if st.button("🔄 Reset Manual Sync", use_container_width=True, key="manual_reset_btn"):
            # Загрузить текущие настройки
            current_settings = _load_sync_settings()
            defaults = _get_sync_defaults()
