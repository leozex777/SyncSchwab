...
def render_positions_table(main_status: Dict, client_manager):
    """Таблица Position Sync Status: Accounts (только Active клиенты)"""
...
def render_sync_history():
    ...
    # Заголовок с checkbox + pin СЛЕВА
    col_pin, col_title = st.columns([0.15, 3])
        ...
        with st.container(height=140):
            for entry in all_history[:10]:
                ...
                if operating_mode == 'simulation':
                    mode_icon = "🔶"
                elif operating_mode == 'live' or not dry_run:
                    mode_icon = "🔴"
                else:
                    mode_icon = "🧪"
