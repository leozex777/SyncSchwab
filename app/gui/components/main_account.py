...
def render():
    ...
    with col1:
        ...
        if st.button("📊 Account Information", type=btn_type1, use_container_width=True, key="main_tab_info"):
            st.session_state.main_account_tab = 'Account Information'
            st.rerun()

    with col2:
        btn_type2 = "primary" if current_tab == 'Edit Main Account' else "secondary"
        if st.button("✏️ Edit Main Account", type=btn_type2, use_container_width=True, key="main_tab_edit"):
